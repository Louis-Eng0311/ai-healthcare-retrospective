...
- 토큰 재발급이 `GET` 중심으로 보이는 구조였습니다.
- 인증 민감 엔드포인트에 요청 제한이 없었습니다.
...
- refresh cookie `SameSite` 설정을 환경별로 적용했습니다.
- 토큰 재발급 `POST /token/refresh`를 추가하고 `GET`은 호환으로 유지했습니다.
- login/refresh/reset-password 구간에 rate limit을 적용했습니다.
- API/HTML 책임 분리는 아직 미완료입니다.
